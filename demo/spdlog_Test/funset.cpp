...
int test_spdlog_console()
{
    try {
        ...
        spd::get("console")->info("loggers can be retrieved from a global registry using the "
        ...
        auto rotating_logger
            = spd::rotating_logger_mt("some_logger_name", "testdata/basic_log", 1048576 * 5, 3);
#else
        auto rotating_logger = spd::rotating_logger_mt(
            "some_logger_name", "../../../testdata/mylogfile_log", 1048576 * 5, 3);
#endif
        for (int i = 0; i < 10; ++i)
...
#else
        auto daily_logger
            = spd::daily_logger_mt("daily_logger", "../../../testdata/daily_log", 2, 30);
#endif
...
int test_spdlog_async()
{
    ...
    // Just call spdlog::set_async_mode(q_size) and all created loggers from now on will be
    // asynchronous..
    size_t q_size = 4096; // queue size must be power of 2
    spdlog::set_async_mode(q_size);
#ifdef __linux__
    auto async_file = spd::daily_logger_st(
        "async_file_logger",
        "/home/ryan/ryan-files/ryan-dev/Todo_Messy_Test/prj/linux_cmake_spdlog_Test/testdata/"
        "async_log");
#else
...
int test_spdlog_err_handler()
{
    ...
    spdlog::set_error_handler(
        [](const std::string& msg) { std::cerr << "my err handler: " << msg << std::endl; });
